...
def loadImageSet(folder=u'E:\data_faces', sampleCount=5): #加载图像集，随机选择sampleCount张图片用于训练
    ...
    for k in range(40):
        ...
        # for i in range(10):
            ...
            # xTrain_pwt = pywt.wavedec2(data[i], 'sym2', mode='symmetric', level=4, axes=(-2, -1))
            # # plt.subplot(1, 2, 2)
        ...
        yTrain= np.tile(yTrain1,4)#   np.tile(a,(2,1))就是把a先沿x轴（就这样称呼吧）复制1倍，即没有复制，仍然是 [0,1,2]。 再把结果沿y方向复制2倍
        yTest=np.tile(yTest1,6)#沿着x轴复制5倍，增加列数
