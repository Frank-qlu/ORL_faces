...
def loadImageSet(folder=u'E:\data_faces', sampleCount=5): #加载图像集，随机选择sampleCount张图片用于训练
    ...
    for k in range(40):
        ...
        yTrain= np.tile(yTrain1,8)#   np.tile(a,(2,1))就是把a先沿x轴（就这样称呼吧）复制1倍，即没有复制，仍然是 [0,1,2]。 再把结果沿y方向复制2倍
        yTest=np.tile(yTest1,2)#沿着x轴复制2倍，增加列数
        #print(yTest.shape)
        """
        yTrain.shape=5,40
        yTest.shape=5,40
...
def main():
    ...
    with tf.name_scope('inputs'):
        ...
        x = tf.placeholder("float", [None, 19],name='x_in')
    ...
    l1 = add_layer(x,19,40,n_layer=1 ,activation_function=tf.nn.sigmoid)
    #定义输出层。此时的输入就是隐藏层的输出——90，输入有90层（隐藏层的输出层），输出有40层
    y= add_layer(l1,40,40,n_layer=2, activation_function=tf.nn.softmax)
    ...
    print("识别率:%.2f%%" %((sess.run(accuracy, feed_dict={x:np.matrix(xTest_pca), y_: np.matrix(yTest)}))*100))
